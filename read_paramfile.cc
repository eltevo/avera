...
char str17[] = "a_start";
char str18[] = "h_max";
char str19[] = "NONISOTROPIC_EXPANSION";
char str20[] = "startH_0";
char str21[] = "FIRST_T_OUT";
char str22[] = "IC_FORMAT";
char str23[] = "LOC_CURV";
char str24[] = "DENSITY_CELLS";
char str25[] = "H_OUT";
...
while(!feof(param_file))
{
	...
	{
		sscanf(c, "%s\t%i", str19, &NONISOTROPIC_EXPANSION);
		if(NONISOTROPIC_EXPANSION > 4)
		{
			printf("\nError: NONISOTROPIC_EXPANSION > 4: no such model. Possible models:\n0=standard cosmology\n1=naive backreaction with boxes\n2=backreaction with voronoi-cells\n3=backreaction with DTFE\n4=naive SPH backreaction\n\n NONISOTROPIC_EXPANSION is set to 4.\n");
			NONISOTROPIC_EXPANSION = 4;
		}
		if(NONISOTROPIC_EXPANSION < 0)
                {
                        printf("\nError: NONISOTROPIC_EXPANSION < 0: no such model. Possible models:\n0=standard cosmology\n1=naive backreaction with boxes\n2=backreaction with voronoi-cells\n3=backreaction with DTFE\n4=naive SPH backreaction\n\n NONISOTROPIC_EXPANSION is set to 0.\n");
                        NONISOTROPIC_EXPANSION = 0;
                }
	}
	if(strstr(c, str20) != NULL)
	{
                sscanf(c, "%s\t%lf", str20, &H0_start);
	...
	{
		sscanf(c, "%s\t%i", str23, &LOC_CURV);
	}
	if(strstr(c, str24) != NULL)
	{
		sscanf(c, "%s\t%i", str24, &DENSITY_CELLS);
	}
